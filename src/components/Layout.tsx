      ...
      <header className="border-b border-[var(--color-border)] bg-[var(--color-surface)] sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2.5 sm:py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-4 min-w-0">
            <Link to="/" className="text-lg sm:text-xl font-bold text-white hover:text-blue-400 no-underline flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <button
            ...
            className="shrink-0 text-xs sm:text-sm bg-blue-600 hover:bg-blue-500 text-white px-2.5 py-1.5 sm:px-3 sm:py-1.5 rounded-lg transition-colors cursor-pointer flex items-center gap-1.5"
      ...
      {showQR && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setShowQR(false)}>
          <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-6 sm:p-8 max-w-sm w-full text-center" onClick={e => e.stopPropagation()}>
            <h2 className="text-white text-lg font-bold mb-2">Open on Mobile</h2>
            ...
            <button
              ...
              className="w-full bg-blue-600 hover:bg-blue-500 text-white py-2.5 rounded-lg text-sm font-medium transition-colors cursor-pointer"
            >
              Close
            </button>
          </div>
